...
public class Player {
    ...
    public interface MyCallback {
        void onCallback(List<ScoreQrcode> qrlist);
    }

    public void callToReadData(){
        readData(new MyCallback() {
            @Override
            public void onCallback(List<ScoreQrcode> qrlist) {
                Log.d(TAG, "THIS: "+String.valueOf(qrlist));
            }
        });
    }
        ...
        qrcodesRef.get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    List<ScoreQrcode> qrlist = new ArrayList<>();
                    initializeStatsFromDB();
                    initializeCodesFromDB();
                    qrlist.addAll(getQrCodes());
                    myCallback.onCallback(qrlist);
                }
            }
        });
    }
